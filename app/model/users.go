...
func (u *usersModel) Login(name string, pass string) *usersModel {
	sql := "select id, username,password,useraccount,role_id,type,status  from g_user where  username=?  limit 1"
	err := u.QueryRow(sql, name).Scan(&u.Id, &u.Username, &u.Pass, &u.UserAccount, &u.Status, &u.RoleId, &u.Type)
	if err == nil {
		// 账号密码验证成功
		if len(u.Pass) > 0 && (u.Pass == pass) {
			return u
		}
	} else {
...
func (u *usersModel) OauthLoginToken(userId int64, token string, expiresAt int64, clientIp string) bool {
	...
	sql := "INSERT INTO `g_oauth_access_token`(user_id,user_token,expire_at,client_ip)" +
		"SELECT ?, ?, ?, ? FROM DUAL WHERE NOT EXISTS(SELECT * FROM g_oauth_access_token WHERE user_id=? AND user_token=?)"
...
func (u *usersModel) OauthCheckTokenIsOk(userId int64, token string) bool {
	sql := "SELECT   token  FROM  `g_oauth_access_tokens`  WHERE   fr_user_id=?  AND  revoked=0  AND  expires_at>NOW() ORDER  BY  updated_at  DESC  LIMIT ?"
...
func (u *usersModel) ShowOneItem(userId int64) *usersModel {
	sql := "SELECT  `id`, `username`, `type`, `role_id`, `status`, `useraccount`  FROM  `g_user`  WHERE `status`=1 and   id=? LIMIT 1"
	fmt.Println(sql)
	fmt.Println(userId)
	rows := u.QuerySql(sql, userId)
	if rows != nil {
		for rows.Next() {
			err := rows.Scan(&u.Id, &u.Username, &u.Type, &u.RoleId, &u.Status, &u.UserAccount)
			if err == nil {
				fmt.Println(u)
				return u
			}
...
func (u *usersModel) Show(username string, limitStart float64, limitItems float64) []usersModel {
	...
	sql := "SELECT  `id`, `username`, `type`, `role_id`, `status`, `useraccount`  FROM  `g_user`  WHERE `status`=1 and   username like ? LIMIT ?,?"
