...
//用户登录
//根据http_code来判断。200为通过，401为登录验证失败
func (u *Users) Login(context *gin.Context) {
	...
	if userModel != nil {
		...
		if userToken, err := userTokenFactory.GenerateToken(userModel.Id, userModel.Username, "13919193998", consts.JwtTokenCreatedExpireAt); err == nil {
			if userTokenFactory.RecordLoginToken(userToken, context.ClientIP()) {
				data := gin.H{
					"userId":     userModel.Id,
					"name":       name,
					"account":   userModel.UserAccount,
					"token":      userToken,
	...
	response.ReturnJson(context, http.StatusUnauthorized, consts.CurdLoginFailCode, consts.CurdLoginFailMsg, "")
...
func (u *Users) GetUserInfo(context *gin.Context){
	...
	if isEffective == consts.JwtTokenOK {
		users := model.CreateUserFactory("").ShowOneItem(userInfo.UserId)
		res := gin.H{
